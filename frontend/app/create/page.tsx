 ...
 *   compose → email (the LAST step, immediately before anything is created)
 *           → reveal (dashboard link + join link).
 *
 ...
 * composed workshop is preserved: we only ever leave the compose state forward.
 */

import { useState } from "react";
import {
  ApiError,
  createPublicWorkshop,
  type WorkshopFull,
} from "@/lib/api";
...
function emailProblem(raw: string): string | null {
  ...
  if (email === "") return "Enter your email address so we can reach you about your workshop.";
  if (email.length < 3 || email.length > 254) return "That email address is too long.";
  if (!EMAIL_RE.test(email)) return "That doesn't look like an email address — check it and try again.";
  return null;
}

type Step = "compose" | "email" | "done";
...
export default function PublicCreatePage() {
  ...
  const submit = async () => {
    ...
    if (problem) {
      ...
      return; // nothing is sent — no row is created by an invalid email
    }
    setEmailError(null);

    const checked = validateComposer(value);
    if (!checked.ok) {
      // Should be unreachable — the composer was validated on the way in.
      setComposeError(checked.error);
      setStep("compose");
      return;
    }
    ...
    try {
      ...
      const { saveLocalWorkshop } = await import("@/lib/localWorkshops");
      saveLocalWorkshop({
        name: workshop.name,
      ...
      if (err instanceof ApiError && err.status === 422) {
        // The server rejects the email far more strictly than it rejects the
        // rest of the body, which the composer already validated.
      ...
      <div data-testid="create-success">
        ...
        <section className="mt-6 rounded-xl border border-stone-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-1 text-sm text-stone-600">
            Participants open it, type their first name, and are straight into the first
            milestone. Nothing to install.
          </p>
        ...
        <div className="mt-8 flex flex-wrap items-center gap-3">
          ...
          <a href="/" className="text-sm font-medium text-stone-600 underline underline-offset-4 hover:text-stone-900">
        ...
        <p className="mt-6 text-sm text-stone-500">
          ...
          shortcut, so keep the link above.
        </p>
      </div>,
    );
  }
  ...
  if (step === "email") {
    return shell(
      <div>
        <p className="text-sm font-medium text-stone-500">Last step</p>
        ...
        >
          <label htmlFor="creator-email" className="mb-1 block text-sm font-medium text-stone-700">
            Your email address
          </label>
          <input
            id="creator-email"
          ...
          <p className="mt-4 text-sm text-stone-500">
            Nothing has been created yet — your workshop is saved the moment you press
            Create.
          </p>
        </form>
      </div>,
  ...
  return shell(
    <div>
      ...
      <p className="mt-3 max-w-xl text-stone-600">
        Name it, then add the milestones your room will work through — markdown, links
        and fenced code blocks all render. You can edit everything later from your
        dashboard.
      </p>

      <div className="mt-8 rounded-xl border border-stone-200 bg-white p-5 shadow-sm sm:p-6">
        <WorkshopComposer value={value} onChange={setValue} idPrefix="pc" />
      </div>
      ...
      <div className="mt-6 flex flex-wrap items-center justify-between gap-3">
        ...
        <Button data-testid="create-continue" onClick={goToEmail}>
          Continue →
        </Button>
      </div>
    </div>,
