...
export function MilestoneManageModal({
  open,
  token,
  milestones,
  onClose,
  ...
  useEffect(() => {
    if (open) setRows(toRows(milestones));
  }, [open, milestones]);
  ...
  const saveAdd = async () => {
    ...
    setError(null);
    try {
      await facilitatorAddMilestone(token, { title, content_md: draft.content_md, minutes });
      setAdding(false);
      onChanged();
    } catch (err) {
  ...
  return (
    <>
      <Modal open={open} onClose={onClose} title="Manage milestones">
        <div className="space-y-4">
          ...
          <ol className="space-y-2" data-testid="manage-milestone-list">
            {rows.map((row, i) => (
              <li key={row.id} className="rounded-lg border border-stone-200 bg-stone-50/60 p-3">
                {editing === row.id ? (
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex shrink-0 items-center gap-1">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => doDelete(row.id)}>
                        Delete
                      </Button>
                    </div>
                  </div>
      ...
      <ConfirmDialog
        ...
        body="This removes the milestone and everyone's completion of it. This can't be undone."
        confirmLabel="Delete"
